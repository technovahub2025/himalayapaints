...
export type PackRow = { packSize: string; quantity: string };

type PackSizeTableProps = {
  onAddRow: () => void;
  onDeleteRow: (index: number) => void;
  onPackSizeChange: (index: number, value: string) => void;
  onQuantityChange: (index: number, value: string) => void;
  packGrandTotal: number;
  packRows: PackRow[];
};
        ...
        <div className="overflow-hidden rounded-3xl border border-line bg-white">
          ...
          <div className="divide-y divide-line">
            {packRows.map((row, index) => {
              ...
              return (
                <div key={`${index}-${row.packSize}`} className="grid gap-3 px-4 py-4 md:grid-cols-12 md:items-center md:gap-4 md:px-5">
                  <div className="md:col-span-4">
                    ...
                    <Input
                      ...
                      step="0.01"
                      value={row.packSize}
                      onChange={(e) => onPackSizeChange(index, e.target.value)}
                      placeholder="5"
                    />
                  </div>
                  ...
                  <div className="md:col-span-5">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                      ...
                      <button
                        ...
                        className="inline-flex h-11 items-center justify-center rounded-2xl border border-line px-4 text-slate-500 transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600 sm:w-11 sm:px-0"
