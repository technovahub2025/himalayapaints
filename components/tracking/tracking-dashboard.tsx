...
export function TrackingDashboard({ email, role }: TrackingDashboardProps) {
  const [productionBatches, setProductionBatches] = useState<ProductionBatch[]>([]);
  const [loading, setLoading] = useState(false);
  const [selectedBatchId, setSelectedBatchId] = useState<string>("");
  const [searchTerm, setSearchTerm] = useState("");
  const [quickFilter, setQuickFilter] = useState<"all" | "today" | "week" | "remarks">("all");
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (cancelled) return;
        if (!productionResponse.ok) throw new Error(productionData.message || "Failed to load production data");
  ...
  const filteredBatches = useMemo(() => {
    ...
    if (!needle) return productionBatches;
  ...
  async function exportBatchExcel(batch: ProductionBatch) {
  ...
  async function exportBatchPdf(batch: ProductionBatch) {
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="w-full max-w-2xl rounded-3xl border border-line bg-white/80 p-4 shadow-sm backdrop-blur">
          ...
          <Input
            ...
            placeholder="Search product, batch no, material, remarks..."
          />
      ...
      <div ref={batchDetailsRef}>
        ...
        <CardBody>
          {selectedBatch ? (
                  ...
                  <tbody>
                    {selectedBatch.lines.length > 0 ? (
                      selectedBatch.lines.map((line, index) => (
                        <tr key={`${selectedBatch._id}-${index}`} className="border-t border-line">
            ...
            <p className="text-sm text-muted">Click View on any batch to inspect details and export that batch.</p>
          )}
        </CardBody>
        </Card>
      </div>
