...
import { AlertCircle, ArrowUpDown, BadgeCheck, BarChart3, Check, ChevronDown, ChevronLeft, ChevronRight, ClipboardList, Copy, Download, Edit2, FileSpreadsheet, FileText, Filter, FolderKanban, LayoutDashboard, LoaderCircle, Plus, Printer, RefreshCw, Save, Search, ShieldCheck, SlidersHorizontal, Table2, Trash2, Upload, Users, X, XCircle } from "lucide-react";
...
function compareByDirection(left, right, direction) {
    const multiplier = direction === "desc" ? -1 : 1;
    return left === right ? 0 : left > right ? multiplier : -multiplier;
}
function getNameCategory(value) {
    const trimmed = String(value ?? "").trim();
    if (!trimmed)
        return "Uncategorized";
    return trimmed.split(/\s+/)[0] || "Uncategorized";
}
function getStatusFromCount(count) {
    return Number(count) > 0 ? "Active" : "Inactive";
}
function formatDateLabel(value) {
    const date = normalizeDateValue(value);
    return date ? ADMIN_DATE_FORMAT.format(date) : "-";
}
...
export function AdminDashboard({ initialItems, initialSection = "dashboard", initialTableName, tableNames }) {
    ...
    const [activeSection, setActiveSection] = useState(["products", "rawMaterials", "analytics"].includes(initialSection) ? initialSection : "dashboard");
    ...
    const [rawMaterialStatusFilter, setRawMaterialStatusFilter] = useState("all");
    const [rawMaterialStartDate, setRawMaterialStartDate] = useState("");
    const [rawMaterialEndDate, setRawMaterialEndDate] = useState("");
    const [rawMaterialSortBy, setRawMaterialSortBy] = useState("name");
    ...
    async function loadAllRawMaterialRecords() {
        const records = [];
        let offset = 0;
        while (true) {
    ...
    async function loadProductSummaries() {
        ...
        const summaries = await Promise.all(tables.map(async (table) => {
            const itemsResponse = await fetch(`/api/admin/items?tableName=${encodeURIComponent(table.name)}`);
            const itemsData = await itemsResponse.json();
            if (!itemsResponse.ok) {
                throw new Error(itemsData.message || `Failed to load ${table.name}`);
            }
            const items = Array.isArray(itemsData.items) ? itemsData.items : [];
            const timestamps = items
                .map((item) => normalizeDateValue(item.createdAt ?? item.updatedAt))
                .filter(Boolean);
            const timeValues = timestamps.map((date) => date.getTime());
            return {
                name: table.name,
                count: Number(table.count ?? items.length),
                category: getNameCategory(formatProductLabel(table.name)),
                status: getStatusFromCount(table.count ?? items.length),
                createdAt: timeValues.length > 0 ? new Date(Math.min(...timeValues)).toISOString() : null,
                updatedAt: timeValues.length > 0 ? new Date(Math.max(...timeValues)).toISOString() : null,
    ...
    useEffect(() => {
        ...
        setActiveSection(["products", "rawMaterials", "analytics"].includes(initialSection) ? initialSection : "dashboard");
    ...
    useEffect(() => {
        ...
        async function loadAdminMetadata() {
            ...
            try {
                const [{ tables, summaries }, materials] = await Promise.all([
                    loadProductSummaries(),
                    loadAllRawMaterialRecords()
                ]);
                if (cancelled)
                    return;
                setAvailableTables(tables);
                setAnalyticsProducts(summaries);
                setRawMaterials(materials);
                setRawMaterialRates(buildRawMaterialRateMap(materials));
                setRawMaterialTotal(materials.length);
                setRawMaterialHasMore(false);
            }
            catch (error) {
                if (!cancelled) {
                    toast.error(error instanceof Error ? error.message : "Failed to load dashboard metadata");
                }
            }
            finally {
                if (!cancelled) {
    ...
    useEffect(() => {
        setProductPage(1);
    }, [searchTerm, productCategoryFilter, productStatusFilter, productStartDate, productEndDate, productSortBy]);
    useEffect(() => {
        setRawMaterialPage(1);
    }, [rawMaterialSearchTerm, rawMaterialCategoryFilter, rawMaterialStatusFilter, rawMaterialStartDate, rawMaterialEndDate, rawMaterialSortBy]);
    const productRecords = useMemo(() => {
        const source = analyticsProducts.length > 0
            ? analyticsProducts
            : availableTables.map((table) => ({
                name: table.name,
                count: table.count,
                category: getNameCategory(formatProductLabel(table.name)),
                status: getStatusFromCount(table.count),
                createdAt: null,
                updatedAt: null,
                totalAmount: 0
            }));
        const needle = searchTerm.trim().toLowerCase();
        const start = normalizeDateValue(productStartDate);
        const end = normalizeDateValue(productEndDate);
        return source.filter((record) => {
            const haystack = [
                record.name,
                formatProductLabel(record.name),
                record.category,
                record.status,
                String(record.count),
            ...
            if (productCategoryFilter !== "all" && record.category !== productCategoryFilter) {
                return false;
            }
            if (productStatusFilter !== "all" && record.status !== productStatusFilter) {
                return false;
            }
            if ((start || end) && !withinDateRange(record.updatedAt ?? record.createdAt, start, end)) {
                return false;
            }
            return true;
        }).sort((left, right) => {
            if (productSortBy === "date") {
                return compareByDirection(normalizeDateValue(left.updatedAt ?? left.createdAt)?.getTime() ?? 0, normalizeDateValue(right.updatedAt ?? right.createdAt)?.getTime() ?? 0, "desc");
            }
            if (productSortBy === "price") {
                return compareByDirection(Number(left.totalAmount ?? 0), Number(right.totalAmount ?? 0), "desc");
            }
            return String(left.name).localeCompare(String(right.name));
        });
    }, [analyticsProducts, availableTables, productCategoryFilter, productEndDate, productSortBy, productStartDate, productStatusFilter, searchTerm]);
    const rawMaterialRecords = useMemo(() => {
        const needle = rawMaterialSearchTerm.trim().toLowerCase();
        const start = normalizeDateValue(rawMaterialStartDate);
        const end = normalizeDateValue(rawMaterialEndDate);
        return rawMaterials.filter((material) => {
            const category = getNameCategory(material.category ?? material.name ?? material.code);
            const status = Number(material.rate ?? 0) > 0 ? "Active" : "Inactive";
            const haystack = [
                material.code,
                material.name,
                category,
            ...
            if (rawMaterialCategoryFilter !== "all" && category !== rawMaterialCategoryFilter) {
                return false;
            }
            if (rawMaterialStatusFilter !== "all" && status !== rawMaterialStatusFilter) {
                return false;
            }
            if ((start || end) && !withinDateRange(material.createdAt ?? material.updatedAt, start, end)) {
                return false;
            }
            return true;
        }).sort((left, right) => {
            if (rawMaterialSortBy === "date") {
                return compareByDirection(normalizeDateValue(left.updatedAt ?? left.createdAt)?.getTime() ?? 0, normalizeDateValue(right.updatedAt ?? right.createdAt)?.getTime() ?? 0, "desc");
            }
            if (rawMaterialSortBy === "price") {
                return compareByDirection(Number(left.rate ?? 0), Number(right.rate ?? 0), "desc");
            }
            return String(left.name).localeCompare(String(right.name));
        });
    }, [rawMaterialCategoryFilter, rawMaterialEndDate, rawMaterialSearchTerm, rawMaterialSortBy, rawMaterialStartDate, rawMaterialStatusFilter, rawMaterials]);
    const productCategories = useMemo(() => Array.from(new Set(productRecords.map((record) => record.category))).sort(), [productRecords]);
    const rawMaterialCategories = useMemo(() => Array.from(new Set(rawMaterialRecords.map((material) => getNameCategory(material.category ?? material.name ?? material.code)))).sort(), [rawMaterialRecords]);
    const dashboardMetrics = useMemo(() => {
        const recentCutoff = new Date();
        recentCutoff.setDate(recentCutoff.getDate() - 14);
        const recentProducts = productRecords.filter((record) => withinDateRange(record.updatedAt ?? record.createdAt, recentCutoff, null)).length;
        const recentRawMaterials = rawMaterialRecords.filter((record) => withinDateRange(record.updatedAt ?? record.createdAt, recentCutoff, null)).length;
        const totalCategories = new Set([
            ...productRecords.map((record) => record.category),
            ...rawMaterialRecords.map((material) => getNameCategory(material.category ?? material.name ?? material.code))
        ]).size;
        return {
            totalProducts: productRecords.length,
            totalRawMaterials: rawMaterialRecords.length,
            totalCategories,
            recentlyAdded: recentProducts + recentRawMaterials
        };
    }, [productRecords, rawMaterialRecords]);
    const productPageCount = Math.max(1, Math.ceil(productRecords.length / ADMIN_PAGE_SIZE));
    const rawMaterialPageCount = Math.max(1, Math.ceil(rawMaterialRecords.length / ADMIN_PAGE_SIZE));
    const paginatedProducts = productRecords.slice((productPage - 1) * ADMIN_PAGE_SIZE, productPage * ADMIN_PAGE_SIZE);
    const paginatedRawMaterials = rawMaterialRecords.slice((rawMaterialPage - 1) * ADMIN_PAGE_SIZE, rawMaterialPage * ADMIN_PAGE_SIZE);
    const filteredTables = productRecords;
    const visibleTables = paginatedProducts;
    function updateAdminUrl(nextTableName = tableName, nextSection = activeSection) {
        const params = new URLSearchParams();
        if (nextTableName) {
            params.set("tableName", nextTableName);
        }
        if (nextSection && nextSection !== "dashboard") {
    ...
    async function importRawMaterialsFromFile() {
        ...
        setRawMaterialImportLoading(true);
        try {
            const response = await fetch("/api/admin/raw-materials/import", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    const sidebarItems = [
        ...
        { key: "products", label: "Products", icon: FolderKanban },
        { key: "rawMaterials", label: "Raw Materials", icon: Table2 },
        { key: "analytics", label: "Analytics", icon: BarChart3 }
    ];
    const recentProductRecords = productRecords.slice(0, 5);
    const recentRawMaterialRecords = rawMaterialRecords.slice(0, 5);
    function handlePrint() {
        window.print();
    }
    return (<div className="grid min-h-screen gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
      <aside className="rounded-[2rem] border border-line bg-white/92 p-5 shadow-soft backdrop-blur lg:sticky lg:top-6 lg:h-[calc(100vh-3rem)] lg:self-start">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted">Himalaya Paints</p>
          </div>
        </div>
        <nav className="grid gap-2">
          {sidebarItems.map((item) => {
            ...
            return (<button key={item.key} type="button" onClick={() => {
                ...
                updateAdminUrl(tableName, item.key);
            }} className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm font-medium transition ${active ? "bg-accent text-white shadow-soft" : "bg-slate-50 text-ink hover:bg-slate-100"}`}>
                <Icon className="h-4 w-4"/>
                {item.label}
              </button>);
        })}
        </nav>
        <div className="mt-6 rounded-[1.5rem] border border-line bg-slate-50 p-4">
          <p className="text-sm font-semibold text-ink">{email ?? "Signed in"}</p>
          <p className="mt-1 text-xs text-muted">Role: admin</p>
          <div className="mt-4 flex flex-wrap gap-2">
            <BadgeCheck className="h-4 w-4 text-accent"/>
            <span className="text-xs text-muted">Live API-backed data</span>
          </div>
        </div>
      </aside>

      <main className="space-y-6">
        <Card className="overflow-hidden border border-line/80 bg-white/85 shadow-soft">
          <CardBody className="flex flex-col gap-4 p-5 sm:flex-row sm:items-end sm:justify-between">
            <div className="max-w-3xl">
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-muted">Admin Workspace</p>
              <Title className="mt-2 text-3xl sm:text-4xl">
                {activeSection === "dashboard" ? "Dashboard" : activeSection === "analytics" ? "Analytics" : activeSection === "products" ? "Products" : "Raw Materials"}
              </Title>
              <p className="mt-2 max-w-2xl text-sm text-muted">
                {activeSection === "dashboard"
                  ? "Review live totals, recent updates, and open the operational views from the sidebar."
                  : activeSection === "analytics"
                    ? "Monitor product and raw material trends without leaving the admin workspace."
                    : activeSection === "products"
                      ? "Manage product tables with the existing editor and table actions."
                      : "Maintain the raw material catalog with inline updates and bulk tools."}
              </p>
            </div>
        ...
        <div className={activeSection === "dashboard" ? "grid gap-4 md:grid-cols-2 xl:grid-cols-4" : "hidden"}>
          <Card><CardBody className="p-5"><p className="text-sm text-muted">Total Products</p><p className="mt-2 text-3xl font-semibold text-ink">{dashboardMetrics.totalProducts.toLocaleString()}</p></CardBody></Card>
          <Card><CardBody className="p-5"><p className="text-sm text-muted">Total Raw Materials</p><p className="mt-2 text-3xl font-semibold text-ink">{dashboardMetrics.totalRawMaterials.toLocaleString()}</p></CardBody></Card>
          <Card><CardBody className="p-5"><p className="text-sm text-muted">Total Categories</p><p className="mt-2 text-3xl font-semibold text-ink">{dashboardMetrics.totalCategories.toLocaleString()}</p></CardBody></Card>
          <Card><CardBody className="p-5"><p className="text-sm text-muted">Recently Added Items</p><p className="mt-2 text-3xl font-semibold text-ink">{dashboardMetrics.recentlyAdded.toLocaleString()}</p></CardBody></Card>
        </div>

        <Card className={activeSection === "analytics" ? "" : "hidden"}>
          <CardHeader>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-lg font-semibold">Analytics</p>
                <p className="text-sm text-muted">A quick snapshot of the current catalog and recent activity.</p>
              </div>
              <p className="text-sm text-muted">{analyticsLoading ? "Refreshing metrics..." : "Live from current API data"}</p>
            </div>
          </CardHeader>
          <CardBody className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <Card><CardBody className="p-5"><p className="text-sm text-muted">Products</p><p className="mt-2 text-3xl font-semibold">{dashboardMetrics.totalProducts.toLocaleString()}</p></CardBody></Card>
            <Card><CardBody className="p-5"><p className="text-sm text-muted">Raw Materials</p><p className="mt-2 text-3xl font-semibold">{dashboardMetrics.totalRawMaterials.toLocaleString()}</p></CardBody></Card>
            <Card><CardBody className="p-5"><p className="text-sm text-muted">Categories</p><p className="mt-2 text-3xl font-semibold">{dashboardMetrics.totalCategories.toLocaleString()}</p></CardBody></Card>
            <Card><CardBody className="p-5"><p className="text-sm text-muted">Recent Additions</p><p className="mt-2 text-3xl font-semibold">{dashboardMetrics.recentlyAdded.toLocaleString()}</p></CardBody></Card>
          </CardBody>
        </Card>
