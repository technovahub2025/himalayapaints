...
export function Button({ className, variant = "primary", type = "button", children, ...props }) {
    ...
    return (<button type={type} {...props} className={cx("inline-flex items-center justify-center rounded-2xl px-4 py-2.5 text-sm font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 disabled:cursor-not-allowed disabled:opacity-50", styles[variant], className)}>
      {children}
    </button>);
}
...
export function Label({ children, ...props }) {
    return <label {...props} className="mb-2 block text-sm font-medium text-ink">{children}</label>;
}
