...
export function ProductSelector({ value, options, onSelect, disabled = false, label = "Product Name", placeholder = "Search products..." }) {
    ...
    return (<div className="relative grid w-full gap-2 sm:min-w-[260px]">
      ...
      <Input type="search" value={query} disabled={disabled} onChange={(e) => {
            ...
            setOpen(true);
        }} onFocus={() => setOpen(true)} onBlur={() => window.setTimeout(() => setOpen(false), 120)} placeholder={placeholder} aria-label={label}/>
      {open ? (<div className="absolute left-0 right-0 top-full z-20 mt-2 max-h-72 overflow-auto rounded-2xl border border-line bg-white shadow-lg">
          {filtered.length > 0 ? (filtered.slice(0, 50).map((option) => {
                ...
                return (<button key={option} type="button" className="flex w-full items-center justify-between px-4 py-3 text-left text-sm transition hover:bg-slate-50" onMouseDown={(e) => e.preventDefault()} onClick={() => {
                        setOpen(false);
